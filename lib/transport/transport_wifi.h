...
//     which mirrors how the eventual LoRa link behaves (no router
//     equivalent exists in the field).
//   - UDP = connectionless, packet-based, matches LoRa's framing
//     model far better than a stateful TCP stream would.
//
// One board (GCS, by convention) runs the AP; the other (rover)
// connects to it as a station. Which role a given board plays is
// controlled by NODE_ROLE_ROVER / NODE_ROLE_GCS build flags.
// ============================================================
...
// to "introduce" itself; the GCS then replies to whatever source IP
// that packet arrived from.
